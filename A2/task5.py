...
def run_NN(file, learning_rate=0.001, momentum=0.0):
	...
	with graph.as_default():
		...
		dhidden = tf.nn.dropout(hidden, 0.5)
		logits = tf.matmul(dhidden, layer2_weights) + layer2_biases
...
fout = open("task5_results.log", 'w', 0)
#fout = sys.stdout
for learning_rate in [0.001, 0.0001]:
	#for momentum in [0.0, 0.5, 0.9]:
	print ("Starting experiment with learning rate {}".format(learning_rate), file=fout)
	run_NN(fout, learning_rate)
fout.close()