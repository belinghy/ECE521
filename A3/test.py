...
seed = 700
embiggen_factor = 70 # Spread factor
...
with tf.Session() as session:
    ...
    for i in range(10):
        nearest_indices2 = assign_to_nearest(samples, updated_centroids)
        updated_centroids = update_centroids(samples, nearest_indices2, n_clusters)
        updated_centroid_value = session.run(updated_centroids)
        plot_clusters(sample_values, updated_centroid_value, n_samples_per_cluster)

    # print "sample_values {} shape: {}".format(type(sample_values), sample_values.shape)
    # print "centroid_values {} shape: {}".format(type(updated_centroid_value), updated_centroid_value.shape)
